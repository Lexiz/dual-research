...
### Cite the live surface, not the dead one

Before citing a function in a spec's `## 2. Proposed change`, **confirm it is reachable from the live entry points** (`run_dr_phase2` / `run_dr_phase4`, via `_drive_interaction_phase`). The reconciler only verifies that a cited `file:line` *exists* — **not** that it is on a live call path. A citation can be literally valid yet point at code dead since the spec-0118 v2 rewrite. Known dead surfaces that read like the live ones:

- `ledger/prompt.py:build_standing_items_section` (and the legacy `orchestrator/phase2.py` / `phase4.py` runners that call it) — the live standing-items surface is `deep_research.format_role_aware_standing_items`, rendered via `dr_run._format_standing_items`.
- The non-`_v2` prompt builders (`negotiation_turn_prompt`, `negotiation_round1_prompt`, `review_turn_prompt`) — the live builders are the `*_v2` variants (`plan_negotiation_round1_prompt_v2`, `plan_negotiation_round_n_prompt_v2`, `review_round1_prompt_v2`, `review_round_n_prompt_v2`).
- The legacy `ledger/models.py:LedgerState` model — the live item state lives on the contract `LedgerEntryV2` machine (`deep_research.py`), which carries the `addressed` lifecycle state the `LedgerState` per-kind vocab does not.

Cite the live surface. If a spec is caught citing a dead one at `/dev-next` time (spec 0257 was — every §2 citation pointed at the legacy path), retarget the implementation onto the live surface and record the dead→live mapping in the PR; do not implement on dead code (that passes string-check CI while the live path regresses — the 0231→0238 trap).

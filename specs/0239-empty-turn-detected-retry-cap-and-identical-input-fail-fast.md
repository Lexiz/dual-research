...
- [ ] **Integration on captured 142625 fixture.** Run the verifier against the **`transcript.captured.jsonl`** copy of the fixture (NOT the working `transcript.jsonl` which spec 0240 will regenerate). I2.7 returns `pass` (only one `empty_turn_detected` event is captured before the kill; cap is not breached, identical-input fail-fast not triggered because retry never completed). This validates the fixture is consistent with the new invariant and is not surfaced as a false positive. The `transcript.captured.jsonl` file is the immutable evidence anchor for the dead-state orchestrator events (`empty_turn_detected`, retry `turn_started`) that turn-file replay cannot reconstruct — spec 0240 establishes the convention; spec 0239 is its first consumer.

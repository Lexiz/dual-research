...
> **Type:** new-feature  |  **Complexity:** M  |  **Depends on:** 0229 (addressee-obligation invariant), 0216 (raiser-self-address observability)
...
Render `_ADDRESS_RESOLVE_ROLE_CALLOUT` inline in `negotiation_round1_prompt`, `negotiation_turn_prompt`, and `review_turn_prompt` (the phase-4 path that emitted the analogous 7× `raiser_self_address` in run `…164844`), immediately above the `## Status` section, so all interaction rounds carry the same role contract.
...
- **Before** (run `20260530-175809`): phase 2 converges `via_ghost_cap` with 49 phase-2 violations; the addressee (openai) emits zero `### ADDRESS` blocks against claude's `D-plan-c-*` items; the raiser (claude) emits `### ADDRESS`/`### RESOLVE` on its own open items every round.
...
- **Early-deadlock-abort.** Explicitly deferred — this run proves item ownership, not abort-latency, is the binding constraint; promote abort only after a run converges via genuine resolution and still wastes rounds.
...
- [ ] `_ADDRESS_RESOLVE_ROLE_CALLOUT` is present in the rendered output of `negotiation_round1_prompt`, `negotiation_turn_prompt`, and `review_turn_prompt`. (falsifiable)
- [ ] **Live-failure regression (CLAUDE.md spec 0238 discipline):** a real re-run on the `backend-language-choice` brief, scored via `/dr-run-assess`, reports phase 2 `phase_converged via_genuine_resolution` (not `via_ghost_cap` / `via_artifact_promotion`) with **zero** `raiser_self_address`, `resolve_from_non_addressed`, and `agreed_with_open_addressed_items` violations. The PR description carries the run ID and the violation tally. (acceptance evidence)
...
- **A live re-run is non-deterministic and cannot gate CI.** Mitigation: the four unit/prompt-content checks above are the CI gate (deterministic, source-pattern style per spec 0206); the live run is acceptance evidence in the PR, exactly as the user accepted ("a real run OR scripted fixture from this run").

...
In [src/dual_research/contract/verifier.py](src/dual_research/contract/verifier.py) `_check_i2_4` (lines 533-593), change all three `InvariantResult("I2.4", "reporting", …)` returns at lines 590, 592, 593 to `InvariantResult("I2.4", "gating", …)`. The detection logic stays as-is — the function already implements the addressee-obligation check correctly; it just reports today. After the flip, the run-summary aggregator's gating-fail predicate at [src/dual_research/contract/verifier.py:111](src/dual_research/contract/verifier.py:111) will catch I2.4 failures.

- **I2.4 gating semantics mirror I4.4 (from spec 0228): handled-vs-unhandled, not raw-emission.** An AGREED-while-owing emission paired with a corresponding `agreed_with_open_addressed_items` ProtocolViolation is HANDLED → I2.4 PASSES. An AGREED-while-owing emission that counted toward convergence with NO matching ProtocolViolation is UNHANDLED → I2.4 FAILS. Raw emission of AGREED-while-owing is not itself a failure: agents will legitimately emit premature AGREED and get demoted+corrected on healthy post-0229 runs (that's the whole point of the demotion mechanism). The verifier diffs `turn_ended` events carrying STATUS: AGREED against the same round's ProtocolViolation events — handled pairs pass, orphan AGREEDs fail. The dead reference run (`20260526-102321`) fixture fails either way (its AGREEDs are frozen pre-0229, no matching ProtocolViolations). The clean reference run (`20260521-010637`) keeps passing because its AGREEDs never carried open-addressed-at-me debt.
...
- [ ] **Verifier snapshot — failing run.** Regenerate `expected.json` for `tests/fixtures/anchor-runs/20260526-102321-backend-language-choice/`. Assert I2.4 now shows `severity: gating, verdict: fail` with the four items (`D-plan-c-02`, `D-plan-c-04`, `D-plan-c-05`, `Q-plan-c-01`) named in the evidence list.
- [ ] **Verifier snapshot — clean run.** Regenerate `expected.json` for `tests/fixtures/anchor-runs/20260521-010637-dvs-backend-language-choice/`. Assert I2.4 shows `severity: gating, verdict: pass` (no addressee violations in the clean run).

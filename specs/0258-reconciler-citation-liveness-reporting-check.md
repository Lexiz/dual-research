...
Spec 0257's §2 citations ALL pointed at dead code. `ledger/prompt.py:build_standing_items_section` (`src/dual_research/ledger/prompt.py:47`), a proposed new `LedgerState.ratifiable_entries`, and the non-`_v2` prompt builders are reachable only from the LEGACY runners `src/dual_research/orchestrator/phase2.py` and `phase4.py` — NOT from the live entry points `run_dr_phase2` (`src/dual_research/orchestrator/dr_run.py:1015`) / `run_dr_phase4` (`dr_run.py:1533`), which compose standing items through `_format_standing_items` (`dr_run.py:602`, called at `dr_run.py:300`). The legacy surface has been dead since the spec-0118 v2 rewrite. The fix had to be retargeted onto the live surface in-flight during `/dev-next`.

The reconciler reported the spec "clean" because `reconcile_spec` (`scripts/spec_lifecycle/reconcile.py:89`) only classifies whether a cited `file:line` still EXISTS within range — `clean` / `mechanical` / `semantic` / `out_of_tree` — via `_extract_citations` (`reconcile.py:141`) and the per-citation existence check in the `reconcile_spec` loop (`reconcile.py:101`). It has no notion of whether the cited symbol sits on a live call path. The interim guard today is the CLAUDE.md prose rule "cite the live surface, not the dead one," enforced only by operator vigilance — which is exactly what missed on 0257. This spec makes that rule executable as a **reporting** check at the same reconcile-time decision point where the dead→live retarget gets made.

## 2. Proposed change

Add a coarse, name-based caller-chain heuristic to `scripts/spec_lifecycle/reconcile.py` that surfaces a **WARN-level reporting flag** for any Python-function citation in a spec's `## 2` whose symbol is not reachable from the live entry points. This is reporting, NOT a gate — it does not contribute to `has_blocking_drift` (`reconcile.py:85`) and does not halt `/dev-next`.
...
- Add `unreachable: list[Citation]` to `ReconcileReport` (`reconcile.py:71`) and a `classification` value `"unreachable"` (alongside `clean | mechanical | semantic | out_of_tree`, `reconcile.py:67`).
...
**Reporting surface.** Extend `format_report` (`reconcile.py:170`) with an `unreachable (informational): N` block listing each flagged `raw` + note, rendered like the existing `out_of_tree` informational block. The CLI `main` (`reconcile.py:186`) return code is UNCHANGED — `unreachable` never affects exit status.

**Worked dead-vs-live example the test pins.** A §2 citation to `src/dual_research/ledger/prompt.py:47` (`build_standing_items_section`) MUST land in `report.unreachable` (reachable only from `phase2.py`/`phase4.py`, which `dr_run.py` imports only `Phase2Outcome`/`Phase4Outcome` from — not the builder). A §2 citation to `src/dual_research/orchestrator/dr_run.py:1015` (`run_dr_phase2`) or `dr_run.py:602` (`_format_standing_items`) MUST NOT be flagged — the first is an entry point itself, the second is reached from it at `dr_run.py:300`.
...
**Before:** `/dev-next` reconcile reports a spec citing `ledger/prompt.py:build_standing_items_section` as fully clean; the operator has no signal the surface is dead until implementation fails on the wrong function.

**After:** reconcile additionally prints `unreachable (informational): 1 — src/dual_research/ledger/prompt.py:47 — citation not reachable from live entry points — possible dead-surface citation`, at the same step where the retarget decision is made; exit status unchanged, `/dev-next` proceeds.
...
- **Deleting the dead legacy surfaces** (`phase2.py` / `phase4.py` / `ledger/prompt.py:build_standing_items_section`). Flagged for deletion elsewhere; not this spec.

## 6. Test plan

New stdlib-only unit test `tests/test_spec_0258_reconcile_liveness.py`:

- [ ] Feeds `reconcile_spec` (or a focused `_check_citation_liveness` entry) a synthetic spec whose `## 2` cites `src/dual_research/ledger/prompt.py:47` (`build_standing_items_section`, a known-dead symbol) and asserts the citation appears in `report.unreachable` with the dead-surface note.

...
3. If `declared > registered`, emit an Evidence row of the form `phase {p} r{r} {agent}: RAISED_THIS_TURN declared {declared}, item_raised events registered {registered}`. Do **not** flag when `declared <= registered`.
...
**Directional, not equality.** Per Cowork's 2026-05-27 adjudication brief (`cowork/briefs/2026-05-27-0232-i2-6-directional-adjudication.md`): only `declared > registered` is the bug class this invariant is meant to catch — the failing-run signature where an agent claimed N raises in its STATUS footer but the ledger registered fewer. `declared < registered` is benign under-reporting — an OpenAI-style `[pending]` placeholder or empty STATUS array stands in for N actual raises, and the registration ledger is the truth. Flagging under-reporting would false-positive on every OpenAI turn using the placeholder convention without surfacing any real failure. Coincidental count-match cases (e.g. 1 declared and 1 registered for entirely different items) are not in scope here — I4.4 + I2.5 cover transition coherence on the registered items.

**Directional rule supersedes the 3.d "count comparison, NOT ID-set comparison" framing.** The original 3.d framing kept the count-vs-set decoupling intent — directional comparison fully achieves it: no ID-shape inspection at all, no sentinel detection, fully ID-shape-agnostic. The 142625 phase 2 r1 claude failure (declared 5 slug-shaped IDs, registered 0) demonstrates why a canonical-ID-skip guard would have been wrong: it would have skipped a real drop. The directional rule catches it because it never inspects ID shape.
...
- [ ] **Positive synthetic — count match** — build a transcript where `count(RAISED_THIS_TURN) == count(item_raised)` per `(phase, round, agent)` for at least one phase-2 turn → `_check_i2_6` returns `InvariantResult("I2.6", "reporting", "pass")`.
- [ ] **Positive synthetic — benign under-reporting** — build a transcript where `count(RAISED_THIS_TURN) == 0` and `count(item_raised) == 8` for some turn (the OpenAI-`[pending]` placeholder shape) → `_check_i2_6` returns `"pass"`. Under-reporting is benign per the directional rule.
- [ ] **Negative synthetic — drop class** — build a transcript with `count(RAISED_THIS_TURN) == 6` and `count(item_raised) == 0` for `(phase=2, round=1, agent="claude")` → `_check_i2_6` returns `"fail"` with an Evidence row naming `(phase 2 r1 claude, declared 6, registered 0)`.
- [ ] **Backwards-compat on clean reference run** — running the full verifier on `tests/fixtures/anchor-runs/20260521-010637-dvs-backend-language-choice/` reports I2.6 as `pass`. Every turn in the clean fixture satisfies `declared <= registered` under the directional rule. `expected.json` for the clean fixture grows by exactly one I2.6 entry.
- [ ] **Snapshot on failing 054652 fixture** — running the full verifier on `tests/fixtures/anchor-runs/20260527-054652-backend-language-choice/` reports I2.6 as `fail` with an Evidence row containing the exact tuple `(phase 2 r1 claude, declared 6, registered 0)`. `expected.json` for the failing fixture updates accordingly.
- [ ] **Snapshot on failing 142625 fixture** — running the full verifier on `tests/fixtures/anchor-runs/20260527-142625-backend-language-choice/` reports I2.6 as `fail` with an Evidence row containing the exact tuple `(phase 2 r1 claude, declared 5, registered 0)`. The turn's IDs are slug-shaped (`D-go-vs-csharp-21`, `Q-csharp-implicit-penalty`, etc.) and do not match `ID_PATTERN`; the directional rule catches the drop because it never inspects ID shape. A canonical-ID-skip guard would have missed this failure.
- [ ] **`uv run pytest tests/test_verifier.py -q` passes** with all six cases above asserted.
...
- **False-positive on benign under-reporting.** Directional comparison (`declared > registered`) admits exactly the drop-class bug and rejects the placeholder/empty-array class. The earlier equality framing would have flagged every OpenAI turn that used `[pending]` as a placeholder — a false-positive class the directional rule eliminates without any ID-shape inspection. Verified empirically against all five anchor-run fixtures: the directional rule flags exactly the two known-failing fixtures' phase-2 r1 claude turns (054652 declared 6 / registered 0; 142625 declared 5 / registered 0) and passes every other turn across the corpus.

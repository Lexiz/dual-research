...
# Spec 0203.1 — Fix: dashboard live API + L-spec event vocab parity with queue-state.json
...
- `TOLERATED_NON_STAGE_STEPS` (line 39): add `"checkpoint_written"`, `"resume_started"`, and `"promoted_as_next"`. The third is emitted by the `/spec-next` skill (Step 8) when a spec is re-ID'd as a decimal child — surfaced live during 0203.1's own queue-time when this very spec was renamed.
- `STEP_LABELS` (line 58): add `"resume_started": "resuming"`, `"checkpoint_written": "checkpoint"`, and `"promoted_as_next": "promoted as next"`.
...
- `_FEED_STEP_ICON` (line 348): `"resume_started": ("replay", "info")`, `"checkpoint_written": ("bookmark_added", "info")`, `"promoted_as_next": ("arrow_upward", "info")`. All three are Material Symbols already used elsewhere in the codebase tree (Google Symbols catalogue; no DS extension needed — DS check below).
- `_FEED_KICKER` (line 367): `"resume_started": "resumed"`, `"checkpoint_written": "checkpoint"`, `"promoted_as_next": "promoted as next"`.
- `_feed_detail` (line 390): add a branch for `checkpoint_written` → `f"{spec_label} · §{_escape(data.get('next_subsection') or '?')} · {len(data.get('completed_subsections') or [])} done"`. Add a branch for `resume_started` → `f"{spec_label} · resumed at §{_escape(_latest_checkpoint_subsection(spec) or '?')}"` where `_latest_checkpoint_subsection` is a new private helper that scans `spec.events` in reverse for the most recent `checkpoint_written` event and returns its `next_subsection`. (The `resume_started` event's own `data` is empty per [checkpoint.py:188](scripts/spec_lifecycle/checkpoint.py).) Add a branch for `promoted_as_next` → `f"{spec_label} · was {_escape(data.get('was') or '?')}"` reading the `was` field that `/spec-next` Step 8 writes.

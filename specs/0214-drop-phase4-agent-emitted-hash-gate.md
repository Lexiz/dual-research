...
# Spec 0214 — Drop Phase 4 agent-emitted SHA-256 gate; converge on version equality

> **Type:** bug  |  **Severity:** P0 (deadlock)  |  **Affects:** orchestrator phase 4 cross-review convergence
> **Bump:** PATCH — bug fix
> **Evidence:** local run `runs/20260521-010637-dvs-backend-language-choice/` (agents content-agreed at round 3, hard-capped at round 8 chasing the hash); full diagnosis in local `phase4-hash-gate-investigation.md`. Both artifacts live outside git (`runs/` is gitignored); cited for human reference, not as a tracked source.

This is **Spec 1 of 3** in a sequenced bug-fix batch from the phase-4 / phase-2 investigation. The other two — in-round blindness in `_drive_interaction_phase` and raiser self-ADDRESS validator gap — land as separate PATCH specs in this batch.
...
**Expected:** Phase 4 converges in the round both agents emit `STATUS: AGREED` with matching `draft_version` pointers and a terminal ledger. The drafter has not revised the draft this round. The orchestrator records the converged draft's canonical SHA in the `Phase4Complete` event for provenance.
...
**Run-level evidence** (local `runs/20260521-010637-dvs-backend-language-choice/`):
...
**Smoking-gun quote** (local `runs/20260521-010637-dvs-backend-language-choice/phase4/round-07-claude.md`):
...
Replace the agent-emitted `draft_hash` ask with an orchestrator-computed version-equality gate cross-checked against `ctx.state.draft_round`. Persist the orchestrator-computed canonical SHA in the `Phase4Complete` event so provenance is preserved — but anchored to the on-disk file, not the agent's mental math.
...
After the `_drive_interaction_phase` call returns at [`dr_run.py:1404-1416`](src/dual_research/orchestrator/dr_run.py:1404), when `result.converged` is true, read the on-disk draft via `current_draft_path(ctx.session, ctx.state.draft_round)` (helper already in use at [`dr_run.py:1318`](src/dual_research/orchestrator/dr_run.py:1318)) and compute `draft_file_sha256 = hash_draft_content(text)` (helper already exported from `contract/artifacts.py:113`). Thread the SHA into the `Phase4Complete` event the driver emits.

Plumbing: `_drive_interaction_phase` constructs `PhaseConverged` internally in `deep_research.py`; the phase-4 wrapper in `dr_run.py` owns `Phase4Complete` (the phase-4-specific summary event). Provenance lands on `Phase4Complete`, not `PhaseConverged` — keeps the latter phase-agnostic.

**9. `src/dual_research/events/types.py` — extend `Phase4Complete` with a `draft_file_sha256` field.**
...
Update the docstring noting this is the orchestrator-computed `canonical_hash` (smart-quote-folded, whitespace-collapsed SHA-256) of the on-disk draft at the converged round. `None` if no draft file is on disk (hard-cap / artifact-promotion paths set it whenever a draft is present; the contract is documented explicitly).
...
**New test file: `tests/orchestrator/test_spec_0214_phase4_version_gate.py`:**
...
- [ ] **Replay fixture test: `test_replay_round03_from_dvs_backend_language_choice_converges_under_new_gate`** — copy the two round-03 turn transcripts (Claude + OpenAI) from the local failing-run directory into `tests/fixtures/phase4_hash_gate_replay/` and commit them. Parse both with `parse_turn_v2`; build a `ctx` stub with `state.draft_round=3`; assert the new gate returns `True`. Locks in that the actual failing run would now converge at round 3 instead of hard-capping at round 8.
...
- **In-round blindness in `_drive_interaction_phase`** (second agent doesn't see first agent's same-round turn) — deferred to the sibling spec in this batch (draft 003).
- **Raiser self-ADDRESS validator gap** (Claude in round 4 of the failing run misuses the `## Addressing items raised against me` section for own items) — deferred to the sibling spec in this batch (draft 004).
...
- **Risk:** Existing `tests/protocol/test_convergence_spec0089.py:342` is more entangled with phase-4 specifics than the investigation believed.
  **Mitigation:** The test is in the `is_plan_agreed_lenient` suite (phase-2 path) per file inspection. Verify during implementation; if entangled, scope-narrow as documented in §5. Low-likelihood: the spec 0089 tests cover plan-agreement lenient mode, which is structurally phase-2.
...
> ### Fixed
> - **Phase 4 hash-gate deadlock (spec 0214):** the cross-review convergence gate no longer asks each agent to compute a SHA-256 hash of the draft body. Convergence now requires both agents to emit `STATUS: AGREED` with matching `draft_version` pointers anchored against the orchestrator's `ctx.state.draft_round`. The orchestrator computes the canonical draft hash itself and records it in the new `Phase4Complete.draft_file_sha256` field for provenance. Fixes the deadlock observed in the `20260521-010637-dvs-backend-language-choice` run where agents were content-agreed at round 3 but burned through to the hard cap at round 8 because neither could produce a hash matching the orchestrator's `_normalize_for_hash` canonicalization.

## Pointers

- Full investigation: local `phase4-hash-gate-investigation.md` (untracked).
- Failing run: local `runs/20260521-010637-dvs-backend-language-choice/` (untracked; `runs/` is gitignored).
...
- Sibling specs in this batch: in-round blindness on `list_turns` (draft 003), raiser self-ADDRESS validator gap (draft 004).

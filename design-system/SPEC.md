...
**Bar 2** carries: kind tabs in the canonical order **Questions · Disagreements · Issues · Comments** (spec 0167 §2.5, matches `.tl-phase__chips`) — **no leading "All" chip** (spec 0167 §2.6, no active chip = "show all"); clicking an active kind chip deselects it. Per-kind phase counts baked into each tab. Agent segmented filter (All / Claude / GPT) · status segmented filter (All / Open / Resolved / Drift). **Hidden in Σ Summary state.** Both segments use the canonical **`.tab-group-solid` + `.tab-solid`** markup with the lifted-tile contract on active (`background: var(--md-surface)`, `box-shadow: var(--md-elev-1)`). Active state is signalled by the **`data-active="true"` attribute** on the option (per spec 0173 §2.3 + spec 0190 — `.tab-solid.is-active` is fully retired; the `<Tab variant="solid">` branch in shared.jsx now emits `data-active` only). **Every option carries its own count chip** (spec 0173 §2.4 completing spec 0167 §2.2): a trailing `<span class="chip-value">N</span>` where the bare number sits inside the span and the parens are produced via `::before` `(` and `::after` `)` pseudo-elements. **The All count and Drift count must always render even at 0** — chip-stability rule from spec 0167 §2.2, so the button width doesn't fluctuate when drift items appear / disappear mid-run. The Drift option itself is conditionally rendered (hidden when `kindFilter === 'questions'`). At narrow viewports (≤ 1799 px) the `.chip-value` spans collapse via `display: none` (labels stay; parens hide). The legacy `.fgroup` / `.fgroup .ft` markup was retired in spec 0173 §2.3.
...
**Reference baseline currency (spec 0205.1).** The four reference PNGs were refreshed against v1.44.2 (post-spec-0205 anatomy) on 2026-05-24 using the `20260521-010637-dvs-backend-language-choice` canonical run. The full index — capture date, app version, canonical run, last anatomy-shifting spec — lives at `design-system/notion-issues/screenshots/REFERENCES.md`. Any future spec that changes ItemCard anatomy (head, body stacking, source-row shape, lifecycle structure, kind chip palette, or `.item-card__*` CSS chrome) MUST refresh the four PNGs in the same PR AND bump the corresponding row in `REFERENCES.md`. Future parity grids compare against the *current* baseline plus any documented departures recorded inline at the bottom of `REFERENCES.md` — never against drift from an earlier point-in-time snapshot.

**Post-spec-0205 anatomy snapshot.** As of v1.44.2 the references capture:
- Lifecycle leads Issue + Comment expanded bodies (Bug 2). `ItemCardLifecycleSection` is the first child of the expanded body; the pre-0205 standalone `<Markdown text={item.body}/>` block above the lifecycle is gone (`item.body` lives inside the lifecycle's raise-row quote).
- Expanded `SourceRow` renders the excerpt as a tinted `<blockquote>` with `--md-outline` brand-toned left border; URL · FETCHED · SEARCH QUERY render as `(t-overline label) · value` grid rows (Bug 1). The pre-0205 inline-spans + `<pre>` shape is gone.
- `mdi:link-variant` is the canonical sources glyph across both the head Sources chip and the `.item-card__sources-hd` segment header (Bug 3). One glyph, two surfaces.
...
**Live-state agent strip** (spec 0166 §2.5–§2.6, refined by spec 0173 §2.1). When `.as.in-header` carries `.is-live`, four reinforcing signals fire together: (a) the spec-0138 §5.1 gradient sweep across the strip; (b) **the `.activity-dot` flips to `var(--p-info)` and animates with the canonical `pulse-info` halo (`2s ease-in-out infinite`)** — info-blue replaces the earlier per-agent brand pulse (sable / sage), since the strip's tint + 2 px left-border already carry brand identity and the dot's job is purely "this model is doing work"; (c) the live activity phrase from `composeAgentActivity` (`"negotiating · round 4"` / `"reviewing · round 2"` etc.); (d) **a new `--md-elev-2` shadow lift**. The not-live default for `.activity-dot` is `var(--md-outline)` (grey), no animation — the dot's colour is purely CSS-driven from the `.is-live` state; JSX exposes only the `.activity-dot` class. Reduced-motion fallback (per spec 0138 + spec 0166): static surface-container-high background, no sweep, no dot pulse, no box-shadow on the dot — the elev-2 lift and the live phrase are retained as static signals.
